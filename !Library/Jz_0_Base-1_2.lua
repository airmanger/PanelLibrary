...
- Removed popups on fixed position
...
defaultPressedCallback = function(inst, dir)
    ...
    elseif inst.digit ~= nil then
		...
		if inst.dial and inst.tics then
			local t = os.clock()
...
function EncoderType:createInstance(dataref,override)
	local inst = BaseInstance.new(self,dataref,override)
    setCallback("drawCallback",    inst, self, (self.icon:count() > 1 and defaultDrawCallbackSelect or defaultDrawCallbackRotate))
    setCallback("pressedCallback", inst, self, defaultPressedCallback)
    setCallback("releaseCallback", inst, self, defaultReleaseCallback)
    setCallback("cursorCallback",  inst, self, defaultCursorCallback)
    setCallback("updateCallback",  inst, self, nil)
...
--]]
function PanelType:addZoomedPane(zoom,doButtons,cursorSet)
	self.panes.zoomed = { zoom = zoom, doButtons = doButtons, cursor = cursorSet}
	return self
end
...
function Panel.new(def,name,x,y,width,height)
	local self = { name = name, def = def, file = string.gsub(def.file, "%%%%", name), rect = Rectangle.new(x,y,width,height), items = {}, panes = {} }
    setmetatable(self,Panel)
    return self
end

function Panel:initPanes()
    local pagebounds
	if self.page == nil or self.page.rect == nil then 
        pagebounds = Rectangle.new(0,0,WIDTH,HEIGHT)
    else
        pagebounds = self.page.rect
    end
    
    local d = self.def.panes.default
	local z = self.def.panes.zoomed
	self.panes.default = { x = pagebounds.x + (self.rect.x > 0 and (self.rect.x - self.rect.width * .5) or -self.rect.x), 
	                       y = pagebounds.y + (self.rect.y > 0 and (self.rect.y - self.rect.height * .5) or -self.rect.y), 
						   width = self.rect.width, height = self.rect.height, zoom = d.zoom, doButtons = d.doButtons}
	...
	if z ~= nil then
		...
		if zi.x < pagebounds.x then zi.x = pagebounds.x elseif zi.x + zi.width  > pagebounds.width  then zi.x = pagebounds.width  - zi.width  end
		if zi.y < pagebounds.y then zi.y = pagebounds.y elseif zi.y + zi.height > pagebounds.height then zi.y = pagebounds.height - zi.height end

		self.panes.zoomed = zi
		
		if z.shadow ~= nil then
			local s = z.shadow; local o = s.offset
			zi.shadow = { file = string.gsub(s.file, "%%%%", self.name), x = zi.x-o, y = zi.y-o, height = zi.height+2*o, width = zi.width+2*o }
		end
	end
    return self
end
...
--[[ Panel:getPage(...) 
  Get the Page-object of the panel or a dummy page if no pages are used.
--]]
...
--[[ Panel:setPage(...) 
  Set the Page-object of the panel.
--]]
...
function Panel:create(id)
    ...
    if id == "zoomed" then
		...
		self.panes.zoomed.button  = button_add(EMPTY, EMPTY, self.page.rect.x, self.page.rect.y, self.page.rect.width, self.page.rect.height, inst.zoom, nil)
...
--]]
function Page.new(id, file, startupVisible, rect)
    local self = { id = id, 
                   file = file, 
...
function Page:deploy()
    if self.file ~= nil then
        if self.rect ~= nil then
            self.image = img_add(self.file, self.rect.x, self.rect.y, self.rect.width, self.rect.height)
        else
            self.image = img_add_fullscreen(self.file)
        end
    end
    if self.rect == nil then self.rect = Rectangle.new(0, 0, WIDTH, HEIGHT) end
    ...
    local id = am_variable_create(ref, "INT", self.startupVisible and 1 or 0)
...
function Page:show(isvisible)
    print(self.id .. ".show(" .. tostring(isvisible) .. ")")
    -- show/hide
...
function DataRef:subscribe(obj, valuenum)
    ...
    if self.index ~= nil then
        inst.update = function(val) 
			if DEBUG then print("<=== " .. self.name .. "[" .. self.index .. "] (" .. self.datatype .. ") = " .. tostring(val[self.index])) end
            inst.obj:update(valuenum, val[self.index]) 
        end
    else
        inst.update = function(val) 
            if DEBUG then print("<--- " .. self.name .. " (" .. self.datatype .. ") = " .. tostring(val)) end
