...
--]]
function Page.new(id, group, file, startupVisible, rect)
    local self = { id = id, 
                   group = group, 
                   file = file, 
...
function Page:show(isvisible)
    ...
    -- hide others?
    if isvisible == true and self.group ~= nil then 
        for k,v in pairs(self.group) do
            if v.id ~= self.id then v:show(false) end
        end
    end
    -- show/hide
